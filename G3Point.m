...
if param.iplot==1
    h=figure;
    plot3(ptCloud.Location(:,1),ptCloud.Location(:,2),ptCloud.Location(:,3),'.k','MarkerSize',1);axis equal tight;hold on;axis off
    for j=1:nlabels;  
        try 
...
if param.saveplot==1 && param.iplot==1;
    nom=[param.figurefolder 'fitted_ellipsoids'];print('-djpeg','-r500',nom);
    savefig(nom);
    close;
end
